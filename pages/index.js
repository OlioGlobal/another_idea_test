...
class Slider {
  ...
  bind() {
    ["setPos", "run", "on", "off", "resize", "mathVals"].forEach(
  ...
  setBounds() {
    ...
    this.sliderInner.style.setProperty(
      "transform",
      `translate3d(${-this.slideWidth}px, 0px, 0px)`
    );
    this.slides.forEach((slide, index) => {
      ...
      slide.style.setProperty(
        "transform",
        `translate3d(${initTranlate}px, 0px, 0px)`
      );
  ...
  setPos(e) {
    ...
    if (this.lerp)
      gsap.to(this, {
        currentX:
          this.offX +
          ((window.innerWidth >= 1024 ? e.clientX : e.touches[0].clientX) -
            this.onX) *
            (window.innerWidth >= 1024 ? 1 : 3),
        duration: 0.1,
      });
    else
      this.currentX =
  ...
  mathVals(translationVal, slide, index) {
    ...
    if (translationVal < this.slideWidth * itemCount) {
      transformOrigin = mapVal(
        0,
        this.slideWidth * itemCount,
        ranges[0][0],
        ranges[0][1],
        translationVal
      );
    ...
    if (translationVal < this.slideWidth) mobTranslation = 0;

    if (translationVal > this.slideWidth * 1) opacity1st = 1;
  ...
  run() {
    ...
    this.slides.forEach((slide, index) => {
      let translationVal = gsap.utils.wrap(
        this.min,
        this.max,
        this.initTranslates[index] + this.lastX
      );
      ...
      if (Math.round(translationVal / this.slideWidth) == 1) {
        if (index != this.prevIndex) {
          document
            .querySelectorAll(".SliderBullets-item")
            .forEach((slideBulletItem, slideBulletItemIndex) => {
      ...
      if (
        0 <= translationVal / this.slideWidth &&
        translationVal / this.slideWidth <= 5
      )
        slide.style.setProperty("visibility", "visible");
      else slide.style.setProperty("visibility", "hidden");

      let transformScaleDispOpacVal = this.mathVals(
        translationVal,
        slide,
        index
      );
      ...
      slide
        .querySelector(".Works-slideContent")
        .style.setProperty("opacity", `${transformScaleDispOpacVal[3]}`);

      sliderImg.style.setProperty(
        "transform-origin",
        `${transformScaleDispOpacVal[0]}% 50%`
      );
      sliderImg.style.setProperty(
        "transform",
        `scale(${transformScaleDispOpacVal[1]})`
      );
      sliderContent.style.setProperty(
        "transform",
        `translate3d(${transformScaleDispOpacVal[2]}vw, 0px, 0px)`
      );
      sliderImageInner.style.setProperty(
        "transform",
        `translate3d(${transformScaleDispOpacVal[3]}vw, 0px, 0px)`
      );
    });
...
function WorksSliderItem(props) {
    ...
    if (!executed.current) {
      props.addPopup(
        <div className={"Slide-popup popup-hidden Slide-Popup-" + props.index}>
          <div className="Popup-frame">
            <div className="Popup-cross">
              close <img alt="cross button" src="/assets/delete-sign--v2.png" />{" "}
            </div>
        ...
        () => {
          ...
          document
            .querySelector(
              ".Works-SlideItem-" + props.index + " .Works-slideContent"
            )
            .addEventListener("click", (e) => {
              document.querySelector(
                cl_name + " .Popup-img"
              ).innerHTML = `<Image src=${props.src} alt=${props.label} className="AppImage-image" />`;

              document.querySelector(cl_name).classList.remove("popup-hidden");
              document
                .querySelector(".Header")
                .classList.add("Header-under-element");
              e.preventDefault();
            });
          ...
          let popup_r_fn = (e) => {
            ...
            document
              .querySelector(".Header")
              .classList.remove("Header-under-element");
            e.cancelBubble = true;
          };

          // removing the popup
          document
            .querySelector(cl_name + " .Popup-cross")
            .addEventListener("click", popup_r_fn);
  ...
  return (
    <div
      className={"Works-slide Works-SlideItem-" + props.index}
    ...
    >
      ...
      >
        ...
        >
          <div
            ...
            style={{
              ...
              transform: "translate3d(0px, 0px, 0px) scale(0.875, 0.875)",
            }}
          >
            <div
              className="AppImage Works-slideImageInnerImg loaded lazyload fit-cover"
              style={{
                "--ratio": "0%",
                touchAction: "pan-y",
                transform: "translate3d(0rem, 0px, 0px)",
            ...
            >
              <div
                className="AppImage-overlay"
                style={{ touchAction: "pan-y" }}
              ></div>
              <div
                className="AppImage-placeholder"
                style={{
                  touchAction: "pan-y",
                  backgroundColor:
              ...
              ></div>
              <picture style={{ touchAction: "pan-y" }}>
                <Image
                  fill
                  draggable="false"
        ...
        <div
          className="Works-slideContent"
          style={{ touchAction: "pan-y", transformStyle: "preserve-3d" }}
        >
          <div
            className="Works-slideWrapTitle"
            style={{ touchAction: "pan-y" }}
          >
            <span
              data-label={props.label}
              style={{ touchAction: "pan-y" }}
              className="Works-slideTitle colorFill"
            >
              {props.label}
            </span>
...
function Ideasbehindtextitem(props) {
  ...
  return (
    ...
    >
      <div
        data-label={label_left + " " + label_left}
        data-label-secondary={label_right + " " + label_left}
...
function VisionbgItem(props) {
  return (
    <div
      className="Vision-bgItem"
      style={{ transform: "scale(0.6, 0.6)", "z-index": 0, opacity: 0 }}
    >
      <div className="Vision-bgItemWrapImage">
        <div
          className="AppImage Vision-bgItemImage loaded lazyload fit-cover"
          style={{ "--ratio": "0%" }}
        >
          <div className="AppImage-overlay"></div>
          <div
            className="AppImage-placeholder"
            style={{ "background-color": "rgb(68, 68, 151)" }}
          ></div>
          <picture>
            <Image
              fill
    ...
    if (!executed.current) {
      ...
      if (true) {
        const worksSlider = new Slider({
          m_cursor_states: props.m_cursor_states,
        });
        worksSlider.init();
        ...
        if (window.innerWidth >= 1024) {
          ...
          gsap.set(
            document.querySelectorAll(
              `${m_h_border_cl[0]} .Works-slideSubtitle, ${m_h_border_cl[0]} .Works-slideOffice`
            ),
            { opacity: 0, y: `${2 * 0.69}vw` }
          );

          // Border
          m_h_border_cl.forEach((cl) => {
            document.querySelectorAll(cl).forEach((slideText) => {
              let work_slideimg_arr = [
                ...document.querySelectorAll(".Works-slideImage"),
              ];
              ...
              slideText.addEventListener("mouseenter", () => {
                ...
                lower_txt_tl.to(
                  slideText.querySelectorAll(
                    `.Works-slideSubtitle, .Works-slideOffice`
                  ),
                  { duration: 0.3, opacity: 1, stagger: 0.1, y: `${0}vw` }
                );
                ...
                t_work_s_arr.splice(
                  work_slideimg_arr.indexOf(
                    slideText.parentElement.querySelector(".Works-slideImage")
                  ),
                  1
                );

                gsap.to(t_work_s_arr, { duration: 0.5, opacity: 0.3 });
              });
              slideText.addEventListener("mouseleave", () => {
                ...
                gsap.to(work_slideimg_arr, { duration: 0.5, opacity: 1 });
              });
            });
          });
        }
    ...
    <div data-scroll-container>
      ...
      <section className="ideasInMotion">
        ...
        <div className="Works-slider">
          ...
          {/* pop up */}
          <div className="slide-popup-list">{popupList}</div>
        </div>
        ...
        <div className="Vision">
          <div className="Vision-bgCross">
            <div className="BackgroundCross in-view">
              <div className="BackgroundCross-wrapper">
                <div
                  className="BackgroundCross-inner"
                  style={{ opacity: 0 }}
                ></div>
              </div>
            </div>
          ...
          <div className="Vision-bg" style={{ opacity: 1 }}>
            ...
            <div className="Vision-bgItems">
              <VisionbgItem
                alt="National Clients"
                imgSrc="/assets/text_slides_thumbnail/1.png"
              />
              <VisionbgItem
                alt="International Clients"
                imgSrc="/assets/text_slides_thumbnail/8.jpg"
              />
              <VisionbgItem
                alt="Partnered Consultants"
                imgSrc="/assets/text_slides_thumbnail/4.png"
              />
              <VisionbgItem
                alt="Acres Associated"
                imgSrc="/assets/text_slides_thumbnail/5.png"
              />
              <VisionbgItem
                alt="Years of Legacy"
                imgSrc="/assets/text_slides_thumbnail/6.png"
              />
            </div>
          </div>
            ...
            <div className="Vision-item">
              <div className="Vision-itemInner">
                <div className="Vision-itemHead">
                  <h2
                    data-label="Think"
                    className="Vision-itemTitle app-title--regular"
                  >
                    100+
                  </h2>
      ...
      <section className="bottomSection">
        <div className="footerLearnMore">
          ...
          <p className="onlyDesk">
            We are India’s only advertising & advisory agency focusing solely on
            the real-estate sector.
            <br />
            We have a proven repertoire of insights & knowledge pertaining the
            industry & its markets.
            <br />
            We offer a complete spectrum of services that enable brands to
            interface with target audiences at multiple touch points.
          </p>

          <p className="onlyMob">
            We are India’s only advertising & advisory agency focusing solely on
            the real-estate sector. We have a proven repertoire of insights &
            knowledge pertaining the industry & its markets. We offer a complete
            spectrum of services that enable brands to interface with target
            audiences at multiple touch points.
          </p>
