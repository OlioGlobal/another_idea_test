...
export default function Contact() {
    ...
    css_selectors.forEach((c_s, index) => {
      document.querySelector(c_s).addEventListener("input", (e) => {
        ...
        if (index == 0) {
          ...
          try {
            let str_last_el = e.target.value
              .split("")
              [e.target.value.length - 1].toString()
              .toLowerCase();

            if (str_last_el < "0" || str_last_el > "9") {
              e.target.value =
                e.target.value.substring(0, e.target.value.length - 1) +
                e.target.value.substring(e.target.value.length);
            }

            if (e.target.value.length > 10) {
              e.target.value = e.target.value.substring(
                0,
                e.target.value.length > 10 ? 10 : e.target.value.length
              );
            }
          } catch (error) {}
        } else if (index == 2) {
  ...
  return (
    <div className="Contact-page" data-scroll-container>
      <Head>
        <title>Contact Us - Another Idea</title>
      </Head>

      <section className="Contact-fields">
        <div
          className="Menu Contact-menu"
          style={{ background: "rgb(85, 65, 248)" }}
        >
          <Link href={"/"}>
            <button
              aria-label="open menu"
              className="MenuButton Header-button ContactMenu-button colorFill menu-open"
              style={{
                "--x": "30.2695px",
                "--y": "98.2717px",
                "--r": "0px",
          ...
          <div className="Menu-wrapper">
            <div className="ContactForm-wrapper">
              <div className="ContactForm-title">Let&apos;s Talk</div>
              <div className="ContactForm">
                <div className="ContactForm-details">
                  ...
                  <div className="ContactDetailsItem">
                    ...
                    <span className="ContactDetailsInfo">
                      Benefice Business House, 4th Floor, 4A, Mathuradas Mill
                      Compound, Tulsi Pipe Road, Lower Parel, Opp. Kamala Mills,
