    ...
    <div className="Menu-navItem">
      ...
      <Link href={props.href} passHref legacyBehavior>
        <a className="Menu-navItemLink">
          <div
            data-label={props.linkLabel}
            className="Menu-navItemLinkInner colorFill"
            style={{ "--x": "0px", "--y": "0px", "--r": "0px" }}
          >
            {props.linkLabel}
            <span
              data-label={props.linkLabel}
              className="Menu-navItemLinkInnerHover"
              style={{ color: "rgb(85, 65, 248)" }}
            ></span>
          </div>
        </a>
      </Link>
        ...
        <div className="Menu-subnav">
          <div>
            <svg
              ...
              className="Menu-subnavArrow icon sprite-icons"
            >
              <path d="M0 0h2v10H0V0z" />
              <path d="M10 9v2H0V9h10z" />
              <path d="M13 10l-4.5 4.33V5.67L13 10z" />
            </svg>
          </div>
          <div className="Menu-subnavItem">
            <a href="/work/branding" className="Menu-subnavItemLink">
              Branding
            </a>
          </div>
          <div className="Menu-subnavItem">
            <a href="/work/campaigns" className="Menu-subnavItemLink">
              Campaigns
            </a>
          </div>
          <div className="Menu-subnavItem">
          ...
          <div className="Menu-subnavItem">
            <a href="/work/strategy" className="Menu-subnavItemLink">
              Strategy
            </a>
          </div>
          <div className="Menu-subnavItem">
            <a href="/work/workshops" className="Menu-subnavItemLink">
              Workshops
            </a>
          </div>
        </div>
...
function MyApp({ Component, pageProps }) {
  ...
  const [pageMicroHistoryState, setPageMicroHistory] = useState({
    prevPage: "",
    currentPage: "",
  });
  ...
  function getRelativePos(element) {
    ...
    return [
      mouse_pos.current[0] - element_pos.left,
      mouse_pos.current[1] - element_pos.top,
    ];
  }
  ...
  const headerAnimation = () => {
    ...
    const setHeaderscrollY = (scrollY) => {
      ...
      if (scrollY >= stickyDisplacement) {
        // just after scrolled above trigger -_
        if (lastscrollY <= stickyDisplacement) {
          ...
          gsap.fromTo(
            ".MenuButton",
            { scale: 0 },
            { duration: 0.5, scale: 1, ease: "circ" }
          );
          headerTriggerStart.current = "init";
        }
      } else if (
        scrollY < stickyDisplacement &&
        lastscrollY >= stickyDisplacement
      ) {
        // just after scrolled below trigger -^
        gsap.set(".Header", { y: (-scrollY).toString() + "px" });
        document.querySelector(".Header").classList.remove("fixed");
        headerTriggerStart.current = "fixed";
      } else if (scrollY < stickyDisplacement) {
        // inbetween scrolling init and fixed =-
        gsap.set(".Header", { y: (-scrollY).toString() + "px" });
      }

      if (scrollY < stickyDisplacement && scrollY > 0) {
        // inbetween scrolling init and fixed =-
      ...
      if (router.asPath == "/work") {
        gsap.set(".List-popup", { y: scrollY });
      }
      if (router.asPath == "/work/branding") {
        gsap.set(".List-popup", { y: scrollY });
      }
      if (router.asPath == "/work/campaigns") {
        gsap.set(".List-popup", { y: scrollY });
      }
      if (router.asPath == "/") {
        gsap.set(".Slide-popup", { y: scrollY });
      }
    ...
    const relateToScrollY = (getScrollY) => {
      ...
      if (document.body.clientWidth > 1023) {
        ...
        styleEventListener.observe(scroller, {
          attributes: true,
          attributeFilter: ["style"],
        });
      } else if (document.body.clientWidth <= 1023) {
        // evidently it's for mobile
        const onScrollFunc = () => {
          scrollY =
    ...
    relateToScrollY((scrollY) => {
      ...
      if (router.asPath.indexOf("/work") != -1) {
        ...
        const WORKSCROLL_LEVEL =
          (WORKNAV_COMPUTED_STYLE.top + WORKNAV_COMPUTED_STYLE.height) * 1.5;
  ...
  const menuButton = () => {
    ...
    let toggleMenuState = () => {
      ...
      if (menuState.current) {
        ...
        const gsapTimelineAnimation = gsap.timeline({
          defaults: {
            duration: 0.7,
            ease: "power3.out",
          },
        ...
        gsap.set(menu_el, {
          "--x":
            (
              menuButtonBoundingRect.left +
              menuButtonBoundingRect.width / 2
            ).toString() + "px",
          "--y":
            (
              menuButtonBoundingRect.top +
              menuButtonBoundingRect.height / 2
            ).toString() + "px",
        });
        gsap.fromTo(
          menu_el,
        ...
        menu_nav_el.forEach((element, index) => {
          ...
          if (index == 0) {
            delay = "<0.2"; //"<0.25"
            if (headerTriggerStart.current == "noheader") delay = "<0.75";
          } else {
            delay = "<0.01";
          }
          ...
          gsapTimelineAnimation
            .fromTo(
              element.querySelector(".Menu-navItemNum"),
              {
                x: element.querySelector(".Menu-navItemNum").clientWidth * 14,
                opacity: 0.1,
              },
              { x: 0, opacity: 1 },
              delay
            ...
            .fromTo(
              element.querySelector(".Menu-navItemLinkInner"),
              {
                x: element.querySelector(".Menu-navItemLinkInner").clientWidth,
              },
              { x: 0, opacity: 1 },
              "<0.1"
            );
        ...
        gsapTimelineAnimation
          .fromTo(
            ...
            {
              x:
                document.querySelector(".Header .Menu-logoContainer")
                  .clientWidth / 3,
              opacity: 0,
            },
            { x: 0, opacity: 1 },
          ...
          .fromTo(
            ".Header .Menu-quickAccessItem",
            {
              x:
                document.querySelector(".Header .Menu-quickAccessItem")
                  .clientWidth / 3,
              opacity: 0,
            },
            { x: 0, opacity: 1 },
          ...
          .fromTo(
            ...
            {
              x:
                document.querySelector(".Header .Menu-socials").clientWidth / 3,
              opacity: 0,
            },
            { x: 0, opacity: 1 },
            "<0.1"
  ...
  const colorFiller = (colorFillsFiltered) => {
    colorFillsFiltered.forEach((element) => {
      ...
      if (element.style.getPropertyValue("--ease-end-duration") != "")
        easeValueOutTime = element.style.getPropertyValue(
          "--ease-end-duration"
        );
      ...
      element.addEventListener("mousemove", (e) => {
        ...
        gsap.to(element, {
          duration: 1,
          "--x": relativePos[0].toString() + "px",
          "--y": relativePos[1].toString() + "px",
        });
  ...
  const m_cursor_states = (state, options) => {
    ...
    if (state == "link") {
      c_tl
        .to(".mouseCursor", {
          ...
          onComplete: () => {
            setm_content_icon(
              <svg
                xmlns="http://www.w3.org/2000/svg"
                className="mouseCursorIcon icon sprite-icons arrow-diag-icon"
              >
                <path
                  xmlns="http://www.w3.org/2000/svg"
                  d="M8.307 10.89h9.472L7.203 21.466l2.207 2.206 10.575-10.575v9.472h3.12V7.771H8.307v3.12z"
                />
              </svg>
            );
          },
        ...
        .fromTo(
          ".mouseCursorIconWrapper",
          {
            x: -document.querySelector(".mouseCursor").clientWidth / 2,
            y: document.querySelector(".mouseCursor").clientWidth / 2,
            opacity: 0,
          },
          { duration: 0.5, x: 0, y: 0, opacity: 1, ease: "back" }
        );
        ...
        .to(".mouseCursor", {
          ...
          onComplete: () => {
            setm_content_icon(
              <svg
                xmlns="http://www.w3.org/2000/svg"
                className="mouseCursorIcon icon sprite-icons move-arrow"
              >
                <path
                  xmlns="http://www.w3.org/2000/svg"
                  d="M12.7 18.7L6 12h15V8.9H6l6.7-6.7L10.5 0 0 10.5l10.5 10.4 2.2-2.2z"
                />
              </svg>
            );
          },
          ...
          onComplete: () => {
            setm_content_icon(
              ...
              >
                <path
                  xmlns="http://www.w3.org/2000/svg"
                  d="M8.3 2.2L15 8.9H0V12h15l-6.7 6.7 2.2 2.2L21 10.4 10.5 0 8.3 2.2z"
                />
              </svg>
            );
          },
        ...
        onComplete: () => {
          setm_content_icon(
            <div className="mouseCursorIcon drag-drop-icon">
              <svg
                className="drag-drop-arrow icon sprite-icons"
                xmlns="http://www.w3.org/2000/svg"
          ...
          let i = setInterval(() => {
            if (document.querySelectorAll(".drag-drop-arrow") === undefined)
              return;

            try {
              c_tl
                .fromTo(
                  document.querySelectorAll(".drag-drop-arrow")[0],
                  {
                    x: document.querySelector(".drag-drop-arrow").clientWidth,
                    opacity: 0,
                  },
                  { duration: 0.5, x: 0, opacity: 1, ease: "back" }
                )
                .fromTo(
                  document.querySelectorAll(".drag-drop-arrow")[1],
                  {
                    x: -document.querySelector(".drag-drop-arrow").clientWidth,
                    opacity: 0,
                  },
      ...
      options.color =
        options.color == "default" ? "rgb(85, 65, 248)" : options.color;
      c_tl.to(".mouseCursor", {
        duration: 0.5,
  ...
  const m_cursor_eventListeners = () => {
    ...
    if (router.asPath == "/contact")
      ...
      m_cursor_states("color", {
        color: colorIndex[pageIndex[router.asPath.split("/").at(-1)]],
      });
      ...
      cl_h_link[router.asPath].forEach((cl) => {
        let interv_cl_vis = setInterval(() => {
          ...
          cl_querySelections.forEach((h_link, index) => {
            ...
            if (
              el_h_event.current.indexOf(h_link) ===
              -1 /* avoids dublicates of events */
            ) {
              ...
              h_link.addEventListener("mouseenter", (e) => {
                ...
                if (cl == ".ideasBehind-item") {
                  ...
                  m_cursor_states("color", {
                    color: h_link.style.getPropertyValue("--ideasBehindColor"),
                  });
                }
                if (cl == ".Menu-subnavItemLink") {
                  // changes menu background color
                  let menuColorPresets = [
                    "#af37d9",
                    "#de477e",
                    "#f2ad45",
                    "#3ab8c9",
                  ...
                  gsap.to(".Menu-navItemLinkInnerHover", {
                    color: menuColorPresets[index],
                  });

                  gsap.to(
                    "#__next > header > div.Menu > div.Menu-wrapper > div > div.Menu-socials > div:nth-child(2) > a > svg > g > path:nth-child(2)",
                    { fill: menuColorPresets[index] }
                  );
                  gsap.to(
                    "#__next > header > div.Menu > div.Menu-wrapper > div > div.Menu-socials > div:nth-child(1) > a > svg > g > path:nth-child(2)",
                    { fill: menuColorPresets[index] }
                  );
                  gsap.to(
                    "#__next > header > div.Menu > div.Menu-wrapper > div > div.Menu-socials > div:nth-child(1) > a > svg > g > path:nth-child(3)",
                    { fill: menuColorPresets[index] }
                  );
                  gsap.to(
                    "#__next > header > div.Menu > div.Menu-wrapper > div > div.Menu-socials > div:nth-child(1) > a > svg > g > path:nth-child(4)",
                    { fill: menuColorPresets[index] }
                  );
                }
  ...
  const p_scroll_trigger = (type) => {
    ...
    var split_t_anim_cl = {
      ...
      "/": {
        ...
        head_obj: [
          ".ideasTitle > h1",
          ".footerLearnMore > h4, .footerLearnMore > h1",
          "",
          "",
          "",
          "",
        ...
        opacity_obj: [
          ...
          "",
          ".swiper",
          "h2, .swiper-slide-active .trusterSliderItem, .swiper-pagination-bullets",
          "",
          "",
          "",
        ], //.trusterSlider:nth-child(-n+3) .trusterSliderItem
      ...
      "/work/strategy": {
        main_target: [
          ".ListIconTitleText-listItem",
          ".Funnel-opening .TitleTextButton-wrapper",
        ],
        head_obj: ["", ""],
        button_obj: ["", ""],
      ...
      "/about": {
        main_target: [".readyToMake", ".Label", ".footerTrustedBy"],
        head_obj: [".readyToMake-text", ".Label-header > h2", ""],
        opacity_obj: [
          "",
          ".Label-tags, .Label-title > h4",
          "h2, .trustersLoop",
        ],
    ...
    import("splitting").then((Splitting) => {
      ...
      if (split_t_anim_cl[router.asPath] !== undefined) {
        ...
        c_p_l.main_target.forEach((target_bunch, index) => {
          ...
          document
            .querySelectorAll(target_bunch)
            .forEach((cur_target, m_ind) => {
              if (
                document.querySelectorAll(c_p_l.main_target[index]).length > 1
              ) {
              ...
              if (c_p_l.main_target !== undefined) {
                ...
                let anim_f = ({
                  ...
                  if (el.indexOf(target_bunch) === -1)
                    el =
                      target_bunch + " " + el.replace(",", `, ${target_bunch}`);
                  ...
                  if (
                    ["h1", "h2", "h3", "h4", "span"].indexOf(
                      document.querySelector(el).tagName.toLowerCase()
                    ) !== -1
                  ) {
                    tween_p =
                      tween_p !== undefined
                  ...
                  if (c_p_l.main_target[index] == ".readyToMake") {
                    default_tl_to = {
                      ...
                      ...{
                        duration:
                          document.querySelector(el).clientWidth * 0.001,
                        stagger:
                  ...
                  let tl_interv = setInterval(() => {
                    let cur_el = document.querySelectorAll(
                      break_down[0] == ""
                        ? el
                        : el.replace(",", " ." + break_down[1] + ",") +
                            " ." +
                            break_down[1]
                    );
                    ...
                    gsap.set(cur_el, { ...tween_p, ...default_from });

                    sc_anim_tl.to(
                      cur_el,
                      { ...tween_to, ...default_tl_to },
                      delay
                    );
                ...
                let opac_bu_objFunc = (target_bunch) => {
                  // opacity_obj
                  if (
                    c_p_l.opacity_obj !== undefined &&
                    c_p_l.opacity_obj[index] !== ""
                  ) {
                    ...
                    sc_anim_tl.to(
                      el,
                      {
                        ...
                        opacity: 1,
                        duration:
                          c_p_l.opacity_dur[index] !== "undef"
                            ? c_p_l.opacity_dur[index]
                            : "1",
                    ...
                    if (el.indexOf(".trusterSliderItem") != -1)
                      sc_anim_tl.fromTo(
                        ".swiper-slide-active .trusterSliderItem",
                        { scale: 0 },
                        { stagger: 0.05, scale: 1 },
                        "<0"
                      );
                  }
                  ...
                  // button_obj
                  if (
                    c_p_l.button_obj !== undefined &&
                    c_p_l.button_obj[index] !== undefined &&
                    c_p_l.button_obj[index] !== ""
                  ) {
                ...
                // Top Text
                if (
                  c_p_l.head_obj !== undefined &&
                  c_p_l.head_obj[index] != "" &&
                  c_p_l.head_obj[index] != undefined
                ) {
                  anim_f({
      ...
      if (window.innerWidth >= 1024) {
        {
          ...
          let percen_vision =
            ((el_vision.clientHeight +
              parseInt(
                window.getComputedStyle(el_vision).marginTop.replace("px")
              ) +
              0.07 *
                parseInt(
                  window.getComputedStyle(el_vision).marginBottom.replace("px")
                )) /
              window.innerHeight) *
              100 -
            55;
...

          {
            ...
            const anim_img = (img_index_N, range_N) => {
              ...
              let prev_class = undefined,
                next_class = undefined;

              if (this_index != range_N[0]) {
                prev_class = ".Vision-item:nth-child(" + (this_index - 1) + ")";
              } // avoid first image
              let this_class = ".Vision-item:nth-child(" + this_index + ")";
              if (this_index != range_N[range_N.length - 1]) {
                next_class = ".Vision-item:nth-child(" + (this_index + 1) + ")";
              } // avoid last image

              try {
                // as to counter any senseless error
                ...
                let an_prog_val =
                  (scroll_prog * range_N[1] - index_by_array) * 100;
                if (an_prog_val < 0) {
                  an_prog_val = 0;
                } else if (an_prog_val > 100) {
                  an_prog_val = 100;
                }
                ...
                if (
                  an_prog_val <= 5 ||
                  (an_prog_val >= 100 &&
                    this_index == range_N[range_N.length - 1])
                ) {
                  // in the initial range the image this make it remain invisible or // when animation is fully completed the

                  // putting current img to zero state
                  gsap.to(this_class.replace("-item", "-bgItem"), {
                    scale: 0.6,
                    opacity: 0,
                  });
                  ...
                  // playes for once when entering the range
                  ...
                  gsap.to(
                    `${
                      prev_class != undefined
                        ? prev_class.replace("-item", "-bgItem")
                        : ""
                    } ${
                      prev_class != undefined && next_class != undefined
                        ? ", "
                        : ""
                    } ${
                      next_class != undefined
                        ? next_class.replace("-item", "-bgItem")
                        : ""
                    }`,
                    {
                      ...
                      opacity: 0,
                      duration:
                        (this_index == range_N[0] &&
                          !prev_pl_state[index_by_array + 1] &&
                          scroll_vec == +1) ||
                        (this_index == range_N[range_N.length - 1] &&
                          !prev_pl_state[index_by_array - 1] &&
                          scroll_vec == -1)
                          ? 0
                          : 0.5, // as to avoid too much delay while the first scroll happens into the animation
                      onComplete: () => {
                        // setting up the current image as the visible

                        if (scroll_prog * range_N[1] - index_by_array > 1)
                          return;

                        for (let i = 0; i < range_N[1]; i++) {
                          if (i != index_by_array) {
                            gsap.to(Vision_bgItem[i], {
                              scale: 0.6,
                              opacity: 0,
                              duration: 0.5,
                            });
                        ...
                        // to be made fluent
                        gsap.fromTo(
                          this_class.replace("-item", "-bgItem"),
                          { scale: 1.2, opacity: 0 },
                          { scale: 1, opacity: 1 }
                        ); // scale in
                        gsap.fromTo(
                          this_class.replace("-item", "-bgItem") +
                            " .Vision-bgItemWrapImage",
                          { scale: 1 },
                          { scale: 1.2 }
                        ); // scale out
                        ...
                        if (prev_class != undefined) {
                          document
                            .querySelector(prev_class)
                            .classList.remove("active");
                        }
                        document
                          .querySelector(this_class)
                          .classList.add("active");
                        if (next_class != undefined) {
                          document
                            .querySelector(next_class)
                            .classList.remove("active");
                        }
                      },
                    }
                  ...
                  if (prev_class != undefined) {
                    prev_pl_state[index_by_array - 1] = false;
                  }
            ...
            let visionItem_tl = gsap.timeline({
              ...
              scrollTrigger: {
                ...
                onUpdate: (self) => {
                  ...
                  scroll_vec =
                    (scroll_vec / Math.sign(scroll_vec)) * scroll_vec; // getting the direction
            ...
            let last_index = 4;
            gsap.to(
              ".Vision-bgItem:nth-child(index)".replace("index", last_index),
              {
                ...
                onComplete: () => {
                  gsap.to(
                    ".Vision-bgItem:nth-child(index)".replace(
                      "index",
                      last_index
                    ),
                    { scale: 0.6, opacity: 0 }
                  ...
                  document
                    .querySelector(".Vision-item:first-child")
                    .classList.remove("active");
                  document
                    .querySelector(
                      ".Vision-item:nth-child(index)".replace(
                        "index",
                        last_index
                      )
                    )
...

          {
            ...
            if (window.innerWidth >= 1024) {
              init_tl
                .fromTo(
                  ".Vision-bgAnimatedLogoVideo",
                  { scale: "0.0146" },
                  { scale: "1" }
                ) // not visible currently( display: none )
...

          {
            ...
            if (document.body.clientWidth >= 1024)
              gsap.set(".Vision-bg", {
                transform: "translate3d(0px, calc(-57.6vh * 0.69), 0px)",
              });
            gsap.fromTo(
              ".Vision-bg",
              { opacity: 0.9 },
            ...
            gsap.set(".Vision .BackgroundCross-inner", {
              transform: "translate3d(0px, calc(-14vw*0.69), 0px)",
            });
            gsap.fromTo(
              ".Vision .BackgroundCross-inner",
              {
...

          {
            ...
            [...document.querySelectorAll(".Vision-item")].forEach(
              (el, index) => {
                ...
                gsap.set(el.querySelector(".Vision-itemTitle"), {
                  x: `${index_condition * 20 * 0.69}vw`,
                  y: `${15 * 0.69}vw`,
                  opacity: 0,
                });
                gsap.set(el.querySelector(".Vision-itemSubtitle"), {
                  x: `${index_condition * 10 * 0.69}vw`,
                  y: `${0 * 0.69}vw`,
                });
                gsap.set(el.querySelector(".Vision-itemKeyFigures"), {
                  x: `${index_condition * 10 * 0.69}vw`,
                  y: `${0 * 0.69}vw`,
                });
                ...
                visionItem1Text_a
                  .fromTo(
                    el.querySelector(".Vision-itemTitle"),
                    {
                      x: `${index_condition * 20 * 0.69}vw`,
                      y: `${15 * 0.69}vw`,
                      opacity: 0,
                  ...
                  .fromTo(
                    el.querySelector(".Vision-itemSubtitle"),
                    {
                      x: `${index_condition * 10 * 0.69}vw`,
                      y: `${0 * 0.69}vw`,
                      opacity: 0,
                  ...
                  .fromTo(
                    el.querySelector(".Vision-itemKeyFigures"),
                    {
                      x: `${index_condition * 10 * 0.69}vw`,
                      y: `${0 * 0.69}vw`,
                      opacity: 0,
...

        {
...

          {
            ...
            init_tl.fromTo(
              ".ideasBehind-wrapper .BackgroundCross-inner",
              { scale: 1, opacity: 0 },
...

          {
            ...
            gsap.set(".ideasBehind-wrapper .BackgroundCross-inner", {
              transform: "translate3d(0px, calc(-14vw*0.69), 0px)",
            });
            gsap.fromTo(
              ".ideasBehind-wrapper .BackgroundCross-inner",
  ...
  const initialize_triggers = (callBack) => {
    let inter_ref = setInterval(() => {
      if (
        locomotiveScrollInstance.current === undefined &&
  ...
  useEffect(() => {
    ...
    if (true) {
      ...
      let concerned_array = window.location.pathname
        .replace("/", "")
        .split("/");
  ...
  useEffect(() => {
    ...
    pageMicroHistory.current = {
      prevPage: pageMicroHistory.current.currentPage,
      currentPage: router.asPath,
    };
    setPageMicroHistory({
      prevPage: pageMicroHistoryState.currentPage,
      currentPage: router.asPath,
    });
    ...
    if (executed.current == 0) {
      ...
      if (true) {
        ...
        document.addEventListener("mousemove", (e) => {
          ...
          if (
            t.substring(t.indexOf("scale(") + 6, t.indexOf("scale(") + 9) ==
            "0.2"
          ) {
            m_pos_l = [
              -document.querySelector(".mouseCursor").clientWidth / 3,
              document.querySelector(".mouseCursor").clientWidth / 3,
            ];
          } else {
            m_pos_l = [
              10 - document.querySelector(".mouseCursor").clientWidth / 2,
              10 + document.querySelector(".mouseCursor").clientWidth / 2,
            ];
          }
          ...
          gsap.to(".mouseCursor", {
            duration: 0.5,
            x: `${e.pageX + m_pos_l[0]}px`,
            y: `${e.pageY - scrollY + m_pos_l[1]}}px`,
            opacity: 1,
            ease: mouseEase,
          });
        });
      ...
      if (true) {
        const resize_ev = () => {
          document
            .querySelector("html")
            .style.setProperty("--vw-size-px", window.innerWidth / 100);
          document
            .querySelector("html")
            .style.setProperty("--vh", window.innerHeight / 100);
        };
    ...
    if (executed.current >= 0) {
      // On page change
      if (
        pageMicroHistory.current.prevPage !==
        pageMicroHistory.current.currentPage
      ) {
        // Mouse Cursor
        if (true) {
          el_h_event.current = [];
            ...
            setTimeout(() => {
              ...
              if (router.asPath == "/") {
                ...
                if (window.navigator.onLine) {
                  let video_url_str =
                    "https://player.vimeo.com/video/1041057807?h=0a152e67e9&title=0&portrait=0&muted=1&autoplay=1&controls=0&dnt=1&loop=1&transparent=0&background=1&app_id=000001";

                  if (document.body.clientWidth < 450)
                    video_url_str =
                      "https://player.vimeo.com/video/1041057807?h=972d4c7b96&title=0&portrait=0&muted=1&autoplay=1&controls=0&dnt=1&loop=1&transparent=0&background=1&app_id=000001";
        ...
        if (true && document.body.clientWidth >= 1023) {
          ...
          if (executed.current >= 1) {
            var unWantedEl = [
              ...document.querySelectorAll(".Header .colorFill"),
            ];
        ...
        if (
          true &&
          windowListeners.current.listeners != undefined &&
          executed.current > 1
        ) {
  ...
  useEffect(() => {
    ...
    if (true) {
      if (pageMicroHistory.current.currentPage !== "/") {
        document
          .querySelector(".Header .Header-route")
      ...
      <>
        ...
        <div className="mouseCursorContainer">
          <div
            ...
            style={{
              transform: "translate3d(0px -50vh, 0px) scale(0.2)",
              borderColor: "rgb(85, 65, 248)",
              backgroundColor: "rgb(85, 65, 248)",
            }}
          >
            <div className="mouseCursorContent">
              <div className="mouseCursorIconWrapper">{m_content_icon}</div>
        ...
        <header className="Header" key={headerVisibilityState}>
          <div className="Header-wrapper">
              ...
              <a href="/" className="Header-logoContainer">
                ...
                >
                  <g clipPath="url(#clip0_6_14)">
                    <path
                      d="M26.6152 31.8H8.9092L6.35496 37.36H0L17.5936 0H17.941L35.5346 37.36H29.1695L26.6152 31.8ZM24.347 26.87L17.7775 12.56L11.2387 26.87H24.347Z"
                      fill="white"
                    />
                    <path
                      d="M70.7014 0.659985V38H70.4256L46.4974 13.54V37.41H40.6227V0.109985H40.9292L64.8063 24.53V0.659985H70.7014Z"
                      fill="white"
                    />
                    <path
                      d="M77.0462 19.1C77.0381 15.3506 78.1672 11.6833 80.2904 8.56247C82.4136 5.44168 85.4354 3.00789 88.9729 1.56941C92.5105 0.130932 96.4047 -0.247514 100.162 0.482015C103.92 1.21154 107.372 3.01622 110.08 5.66743C112.789 8.31865 114.633 11.6971 115.378 15.3748C116.124 19.0525 115.737 22.8641 114.267 26.3265C112.798 29.789 110.311 32.7466 107.122 34.8247C103.934 36.9028 100.187 38.0079 96.3563 38C91.2432 37.9737 86.3472 35.974 82.7316 32.4353C79.1161 28.8965 77.073 24.1045 77.0462 19.1V19.1ZM109.792 19.1C109.81 16.5002 109.039 13.9537 107.577 11.7831C106.115 9.6125 104.027 7.9156 101.579 6.90742C99.1307 5.89925 96.4316 5.6252 93.8239 6.11999C91.2162 6.61479 88.8173 7.85616 86.9312 9.68678C85.0451 11.5174 83.7566 13.8549 83.2292 16.4029C82.7018 18.951 82.9591 21.595 83.9686 23.9998C84.978 26.4045 86.6941 28.4619 88.8995 29.9111C91.1048 31.3602 93.7 32.136 96.3563 32.14C99.9039 32.1296 103.304 30.7504 105.82 28.3016C108.335 25.8529 109.762 22.5322 109.792 19.06V19.1Z"
                      fill="white"
                    />
                    <path
                      d="M143.64 6.45H133.618V37.36H127.712V6.45H117.7V0.75H143.64V6.45Z"
                      fill="white"
                    />
                    <path
                      d="M177.162 0.75V37.36H171.298V21.9H155.094V37.36H149.25V0.75H155.094V16.19H171.298V0.75H177.162Z"
                      fill="white"
                    />
                    <path
                      d="M191.17 6.41V15.79H205.361V21.51H191.17V31.64H207.578V37.36H185.305V0.75H207.578V6.41H191.17Z"
                      fill="white"
                    />
                    <path
                      d="M235.184 37.36L225.121 23.19H220.523V37.36H214.648V0.75H226.275C233.233 0.75 238.944 6 238.944 12.52C238.957 14.7643 238.249 16.9558 236.92 18.7844C235.591 20.6131 233.708 21.9864 231.537 22.71L242.428 37.36H235.184ZM226.388 18.42C228.054 18.4448 229.664 17.8312 230.874 16.7102C232.084 15.5892 232.799 14.0496 232.865 12.42C232.842 11.6132 232.655 10.8189 232.316 10.0831C231.977 9.34737 231.492 8.68465 230.89 8.13328C230.287 7.5819 229.579 7.15279 228.807 6.87074C228.034 6.58869 227.212 6.45928 226.388 6.49H220.492V18.49L226.388 18.42Z"
                      fill="white"
                    />
                    <path
                      d="M262.157 0.75H268.032V37.36H262.157V0.75Z"
                      fill="white"
                    />
                    <path
                      d="M306.111 19.1C306.111 30.35 298.703 37.36 286.831 37.36H276.175V0.750012H286.831C298.703 0.730012 306.111 7.77001 306.111 19.1ZM300.175 19.1C300.175 11.33 295.015 6.47001 286.801 6.47001H281.988V31.64H286.842C295.046 31.64 300.175 26.8 300.175 19.08V19.1Z"
                      fill="white"
                    />
                    <path
                      opacity="0.6"
                      d="M344.026 34.75L345.385 31.8L354.243 12.56L363.07 31.8H363.091L365.645 37.36H372L354.406 0H354.059L336.465 37.36H342.82L344.026 34.75Z"
                      fill="#BABCBE"
                    />
                    <path
                      d="M318.31 31.64V21.51H335.791L338.488 15.79H318.31V6.41H342.902L345.569 0.75H312.435V37.36H328.322L331.02 31.64H318.31Z"
                      fill="white"
                    />
                    <path
                      d="M334.677 37.36H334.718V37.29L334.677 37.36Z"
                      fill="white"
                    />
                  </g>
                  <defs>
            ...
            <button
              ...
              className="MenuButton Header-button colorFill"
              style={{
                "--x": "30.2695px",
                "--y": "98.2717px",
                "--r": "0px",
          ...
          <div
            className="Menu"
            style={{ background: "rgb(85, 65, 248)", display: "none" }}
          >
            ...
            <div className="Menu-wrapper">
              <nav className="Menu-nav">
                <NavItem itemNum="01" linkLabel="Home" href="/" />
                <NavItem
                  itemNum="02"
                  linkLabel="Work"
                  href="/work/branding"
                  subnav={true}
                />
                <NavItem itemNum="03" linkLabel="About Us" href="/about" />
                <NavItem itemNum="04" linkLabel="Team" href="/team" />
              </nav>
              <div className="Menu-secondNav">
                ...
                <div className="Menu-socials">
                  <div className="Menu-socialsItem">
                    <Link
                      href={"https://www.vimeo.com/anotherideaspaces"}
                      passHref
                      legacyBehavior
                    >
                      <a
                        className="Menu-socialsItemLink"
                        target={"_blank"}
                        rel="noopener noreferrer"
                      >
                        <svg
                          xmlns="http://www.w3.org/2000/svg"
                          className="Menu-socialsItemIcon"
                          width="24"
                          height="24"
                          viewBox="0 0 24 24"
                          fill="none"
                        ...
                        <span className="Menu-socialsItemLabel">Vimeo</span>
                      </a>
                    </Link>
                  </div>
                  <div className="Menu-socialsItem">
                    <Link
                      href={"https://www.instagram.com/anotherideaspaces"}
                      passHref
                      legacyBehavior
                    >
                      <a
                      ...
                      >
                        ...
                        <span className="Menu-socialsItemLabel">Instagram</span>
                      </a>
                    </Link>
                  </div>
                </div>
                ...
                <a className="Menu-logoContainer onlyDesk">
                  ...
                  >
                    <g clipPath="url(#clip0_12_104)">
                      <path
                        d="M18.2104 21.7579H6.09577L4.34813 25.5621H0L12.0377 0H12.2754L24.3132 25.5621H19.958L18.2104 21.7579ZM16.6585 18.3847L12.1636 8.59368L7.68962 18.3847H16.6585Z"
                        fill="white"
                      />
                      <path
                        d="M48.3747 0.451572V26H48.1859L31.814 9.2642V25.5963H27.7945V0.0752563H28.0042L44.3411 16.7837V0.451572H48.3747Z"
                        fill="white"
                      />
                      <path
                        d="M52.7158 13.0684C52.7103 10.5031 53.4828 7.99381 54.9355 5.85853C56.3882 3.72325 58.4558 2.05803 60.8762 1.07381C63.2967 0.089585 65.9611 -0.169351 68.5321 0.3298C71.103 0.828951 73.4647 2.06373 75.3181 3.87772C77.1714 5.6917 78.433 8.00328 78.943 10.5196C79.453 13.036 79.1884 15.6438 78.1828 18.0129C77.1773 20.3819 75.4759 22.4056 73.2943 23.8274C71.1127 25.2493 68.549 26.0054 65.928 26C62.4296 25.982 59.0797 24.6138 56.6059 22.1926C54.1321 19.7713 52.7342 16.4925 52.7158 13.0684V13.0684ZM75.1206 13.0684C75.133 11.2896 74.6057 9.54725 73.6053 8.06211C72.605 6.57698 71.1767 5.41594 69.5015 4.72613C67.8263 4.03633 65.9795 3.84882 64.1953 4.18736C62.4111 4.52591 60.7697 5.37527 59.4792 6.6278C58.1887 7.88033 57.3072 9.47965 56.9463 11.2231C56.5855 12.9665 56.7615 14.7755 57.4522 16.4209C58.1429 18.0663 59.3171 19.4739 60.826 20.4655C62.3349 21.457 64.1105 21.9878 65.928 21.9905C68.3553 21.9834 70.6819 21.0397 72.4028 19.3643C74.1238 17.6888 75.1003 15.4168 75.1206 13.041V13.0684Z"
                        fill="white"
                      />
                      <path
                        d="M98.2803 4.41315H91.4226V25.5621H87.382V4.41315H80.5313V0.513153H98.2803V4.41315Z"
                        fill="white"
                      />
                      <path
                        d="M121.216 0.513153V25.5621H117.204V14.9842H106.117V25.5621H102.118V0.513153H106.117V11.0774H117.204V0.513153H121.216Z"
                        fill="white"
                      />
                      <path
                        d="M130.8 4.38578V10.8037H140.51V14.7174H130.8V21.6484H142.027V25.5621H126.788V0.513153H142.027V4.38578H130.8Z"
                        fill="white"
                      />
                      <path
                        d="M160.916 25.5621L154.03 15.8668H150.884V25.5621H146.865V0.513153H154.82C159.581 0.513153 163.488 4.10526 163.488 8.56631C163.497 10.1019 163.013 11.6013 162.103 12.8525C161.194 14.1037 159.905 15.0433 158.42 15.5384L165.872 25.5621H160.916ZM154.897 12.6032C156.037 12.6201 157.138 12.2003 157.966 11.4333C158.794 10.6663 159.283 9.61286 159.329 8.49789C159.313 7.94584 159.185 7.40241 158.953 6.89899C158.721 6.39556 158.389 5.94213 157.977 5.56487C157.565 5.18761 157.081 4.89401 156.552 4.70103C156.023 4.50804 155.461 4.4195 154.897 4.44052H150.863V12.651L154.897 12.6032Z"
                        fill="white"
                      />
                      <path
                        d="M179.371 0.513153H183.39V25.5621H179.371V0.513153Z"
                        fill="white"
                      />
                      <path
                        d="M209.444 13.0684C209.444 20.7658 204.376 25.5621 196.253 25.5621H188.962V0.513152H196.253C204.376 0.499467 209.444 5.31631 209.444 13.0684ZM205.383 13.0684C205.383 7.7521 201.852 4.42684 196.232 4.42684H192.939V21.6484H196.26C201.873 21.6484 205.383 18.3368 205.383 13.0547V13.0684Z"
                        fill="white"
                      />
                      <path
                        opacity="0.6"
                        d="M235.386 23.7763L236.316 21.7579L242.377 8.59368L248.417 21.7579H248.431L250.178 25.5621H254.526L242.489 0H242.251L230.213 25.5621H234.561L235.386 23.7763Z"
                        fill="#BABCBE"
                      />
                      <path
                        d="M217.791 21.6484V14.7174H229.752L231.597 10.8037H217.791V4.38578H234.617L236.442 0.513153H213.771V25.5621H224.642L226.487 21.6484H217.791Z"
                        fill="white"
                      />
                      <path
                        d="M228.99 25.5621H229.018V25.5142L228.99 25.5621Z"
                        fill="white"
                      />
                    </g>
                    <defs>
