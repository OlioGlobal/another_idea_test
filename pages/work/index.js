...
function GallaryListitem(props) {
        ...
        document.querySelector(cl_name).addEventListener("click", (e) => {
          setPopup(
            LIST_WITH_PDF.indexOf(props.LIST_NAME) !== -1 ||
              !(LIST_WITH_IMG.indexOf(props.LIST_NAME) !== -1) ? (
          ...
          document
            .querySelector(cl_name + " .List-popup")
  ...
  return (
    <div
      className={`List-item ${
        LIST_WITH_RECT_STYLE.indexOf(props.LIST_NAME) !== -1 ? "rect" : "simple"
      }-list-item ${props.LIST_NAME}-list-item list-${props.LIST_NAME}-${
        props.index
      }`}
      ref={rootElRef}
    >
      <div className="WorksListItem in-view">
        <div
          className={`AppImage fit-${
            LIST_WITH_RECT_STYLE.indexOf(props.LIST_NAME) !== -1
              ? "cover"
              : "contain"
          } loaded plane WorksListItem-thumbnail`}
        >
        ...
        <h3 className="WorksListItem-title u-textUppercase app-title--small">
          {props.label}
        </h3>
      </div>
      <div className="List-popup popup-hidden">
        <div className="Popup-frame">
          <div className="Popup-cross">
            close <img alt="cross" src="/assets/delete-sign--v2.png" />
          </div>
          {popup}
        </div>
      </div>
...
export function GallaryList(props) {
  ...
  const makeListItemsAnimated = () => {
    document
      .querySelectorAll(`.${props.LIST_NAME}-list-item`)
      .forEach((listItemEl) => {
        listItemEl
          .querySelector(".AppImage-overlay")
          .style.setProperty("opacity", 1);
        gsap.set(listItemEl.querySelector(".WorksListItem-title"), {
          opacity: 0,
          x: "20%",
        });
        gsap.set(listItemEl.querySelector(".WorksListItem-details"), {
          opacity: 0,
          x: "20%",
        });
        ...
        const worksItemScrollTimeLine = gsap.timeline({
          ...
          scrollTrigger: {
            trigger: listItemEl,
            scroller:
              window.innerWidth > 1024 ? "[data-scroll-container]" : undefined,
            start: "top bottom-=15%",
            end: "top 30%",
          },
        });
        ...
        worksItemScrollTimeLine
          .fromTo(
            listItemEl.querySelector(".AppImage-overlay"),
            { x: "0%" },
            {
              duration: 0.5,
              x: "-200%",
          ...
          .fromTo(
            listItemEl.querySelector(".WorksListItem-title"),
            { opacity: 0, x: "20%" },
            { opacity: 1, x: "0%" },
            "<0.05"
          )
          .fromTo(
            listItemEl.querySelector(".WorksListItem-details"),
            { opacity: 0, x: "20%" },
            { opacity: 1, x: "0%" },
            "<0.1"
          );
    ...
    props.s_trigger_anim(() => {
      ...
      let intervalRef = setInterval(() => {
        ...
        try {
          ...
          s_t_a =
            parseInt(c_s_t.split("(")[1].split(")")[0].split(",")[5] * -1) >
        ...
        if (window.innerWidth > 1024) {
          ...
          if (props.locomotiveScrollInstance.current === undefined || !s_t_a)
            return;
        ...
        if (props.locomotiveScrollInstance.current !== undefined)
  ...
  return (
    <div
      className={`List-items ${props.LIST_NAME}-list`}
